        ...
        for o in data:
            if o.get("speaker") == old_speaker:
                ...
                if is_fordela_replik(o["text"]):
                    o["type_of_speech"] = "fördela replik"
                elif is_fordela_genmale(o["text"]):
                    o["type_of_speech"] = "fördela genmäle"
                else:
                    o["type_of_speech"] = "fördela anförande"
...
for i, obj in enumerate(data):
    ...
    if len(forekommande_namn) > 0:
        for fn in forekommande_namn:
            ...
            if match is not None:
                target_idx = i + 1
                if target_idx < len(data) and data[i].get("role") == "ordförande":
    ...
    elif (is_genmale(data[i-1].get("type_of_speech"))) and (data[i-1].get("role") != "ordförande"):
        if data[i].get("speaker").startswith("SPEAKER") and data[i-1].get("role") != "ordförande":
